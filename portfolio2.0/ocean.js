const TEXTURE_PATH = './public/ocean_tile.png'
const TEXTURE_REPEAT = 20
import * as THREE from 'three'


export default class Ocean { // eslint-disable-line no-unused-vars
  constructor () {
    this.ready = false
    this.step = 0
  }
  ...
  init () {
    return new Promise((resolve) => {
      ...
      loader.load(TEXTURE_PATH, (texture) => {
        ...
        this.geometry = new THREE.PlaneGeometry(256*4, 256*4, 256*4)
...
const VERTEX_SHADER = `
  ...
  void main()
  {
    ...
    newPosition.z = 0.0;
    newPosition.z += sin(6.2 * (position.x + time)) * 1.0;
    newPosition.z += sin(4.3 * (position.y + time)) * 1.0;
